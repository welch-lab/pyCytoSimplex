...
TRIANGLE_VERTICES = pd.DataFrame({'x': [0, 0.5, 1],
                                  'y': [0, SQRT3/2, 0]})
TETRA_VERTICES = pd.DataFrame({'x': [-1/SQRT3, 2*SQRT3/3, -1/SQRT3, 0],
                               'y': [-1, 0, 1, 0],
                               'z': [0, 0, 0, 2*SQRT6/3]})
...
def _check_cluster_vertices(x, cluster_var, vertices, n=None):
    ...
    cluster_var : str, list, or pd.Series
        ...
        When list or pd.Series, the length must equal to adata.n_obs or matrix.shape[0].
    vertices : list or dict
        The terminal specifications of the simplex.
        When list, the length must equal to n if n specified and all elements must be in cluster_var.
        When dist, served as grouped specifications of the terminals, meaning each terminal stands for a group of
            clusters. The number of the keys must equal to n if n specified, and all elements of each value (list) must
            be in cluster_var.
    n : int
    ...
    if isinstance(x, AnnData):
        ...
        raise TypeError(
            "The object must be an AnnData, numpy.ndarray, or scipy.sparse.csr_matrix."
        )
    ...
    if len(cluster_var) != nobs:
        raise ValueError(
            "The length of the cluster_var must match the number of observations."
        )
    # Set category of cluster_var to be categorical.
    cluster_var = cluster_var.astype("category")

    if isinstance(vertices, list):
        # Check list type vertices and should all exist in the categories of cluster_var.
        if not all(vertex in cluster_var.cat.categories for vertex in vertices):
...
def _map_category(x, mapping):
    ...
    for key, value in mapping.items():
        if isinstance(value, str):
            if not value in x.cat.categories:
                raise ValueError(f"{value} is not in the categories of `cluster_var`.")
            map_rev[value] = key
        elif isinstance(value, int):
            map_rev[value] = key
        else:
            for v in value:
                if not v in x.cat.categories:
                    raise ValueError(f"{v} is not in the categories of `cluster_var`.")
                map_rev[v] = key
