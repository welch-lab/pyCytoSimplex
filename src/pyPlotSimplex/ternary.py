...
def plot_ternary(
    ...
    """
    Create ternary plot that shows the similarity between each single cell and the
    three vertices of a simplex which represents specified clusters.

    Parameters
    ----------
    X : AnnData, numpy.ndarray, or scipy.sparse.csr_matrix
        The expression matrix of the single cells. Each row represents a single cell and each column represents a gene.
    cluster_var : str, list, or pd.Series
        The cluster assignment of each single cell.
        Only when `X` is AnnData, can be a str that specifies the name of the cluster variable in `X.obs`.
        list or pd.Series is accepted in all cases, and the length must equal to `X.shape[0]`.
    vertices : list or dict, where the length must equal to 3.
        ...
        When dict, served as grouped specifications of the terminals, meaning each terminal stands for one or more
            clusters. The values can be either a str for a single cluster or a list of multiple clusters.
    features : list, optional
        The features to be used in the calculation of similarity. If None, all features will be used.
    velo_graph : csr_matrix, str
        ...
        When `X` is an AnnData object, `velo_graph` can be a str which is a key in `X.uns` that specifies a sparse
            matrix with shape `(n_obs, n_obs)`.
    ...
    fig_size : tuple of two numbers, optional
        The size of the figure. The first number for width and the second for height. Default: None.
    processed : bool, optional
        Whether the input matrix is already processed. When `False`, a raw count matrix (with integers) is recommended
            and preprocessing of library-size normalization, scaled log1p transformation will happen internally. If
            `True`, the input matrix will directly be used for computing the similarity. Default: False.
    method : str, choose from "euclidean", "cosine", "pearson", or "spearman".
        The method to calculate the similarity. Default: "euclidean". When "euclidean" or "cosine", the similarity is
            converted from the distance with a Gaussian kernel. When "pearson" or "spearman", the similarity is derived
            as the correlation is.
    force : bool
        Whether to force the calculation when the number of features exists 500. Default: False.
    sigma : float
    ...
    split_cluster : bool
        Whether to split the plot by clusters. If False (default), all cells will be plotted in one plot. If True, the
            cells will be split by clusters and each cluster will be plotted in one subplot.
    cluster_title : bool
        Whether to show the cluster name as the title of each subplot when `split_cluster=True`. Default: True.
    dot_color : str
        The color of the dots. Default: "#8E8E8EFF".
    n_velogrid : int
        The number of square grids, along the bottom side of triangle, to aggregate the velocity of cells falling into
            each grid. Default: 10.
    radius : float
        Scaling factor of aggregated velocity to get the arrow length. Default: 0.08.
    dot_size : float
        The size of the dots. Default: 0.6.
    vertex_colors : list of three str
        The colors of the vertex labels, grid lines, axis labels and arrows. Default: ["#3B4992FF", "#EE0000FF", "#008B45FF"],
    ...
    """
    mat, grouping, vertices, original_cluster = _check_cluster_vertices(X, cluster_var, vertices, n=3)
    if not processed:
    ...
    if not split_cluster:
        ...
        _add_ternary_subplot(ax, sim_mat, velo_mat=velo_mat, dot_color=dot_color,
                             n_velogrid=n_velogrid, radius=radius, dot_size=dot_size,
        ...
        for i, cluster in enumerate(cats):
            if velo_mat is not None:
                ...
                velo_mat_sub = None
            ax = fig.add_subplot(subplot_nrow, subplot_ncol, i + 1, projection='ternary')
            _add_ternary_subplot(ax, sim_mat.loc[original_cluster == cluster,:],
        ...
        ax = fig.add_subplot(subplot_nrow, subplot_ncol, i + 2, projection='ternary')
