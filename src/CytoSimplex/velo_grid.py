...
def aggregate_grid_velo(sim_mat, velo_mat, n_grid=10, radius=0.1):
    ...
    # Then expand to space by combining all possible combinations, which
    # derives the cartesian coordinates of the grid centroids.
    ...
    # Select those that fall into the simplex space with barycentric
    # coordinates.
    grid_bary = _cart2bary(simplex, grid_cart)
    grid_bary = pd.DataFrame(grid_bary, columns=sim_mat.columns)
    grid_selection = (grid_bary >= -1e-10).all(1) & \
        (grid_bary <= 1+1e-10).all(1)
...
def _cart2bary(X, P):
    """
    X: simplex cartesian coordinates, vertices on row and dimensions on
       column
